...
CLASS_INDEX = {v: k for k, v in cfg.dataset.classes.items()}
...
def create_iou_table(samples, outputs, predictions, class_labels):
    ...
    def _to_str(list_):
        return [f"{str(x)} IoU" for x in list_]

    items = list(zip(samples, outputs, predictions))

    table = wandb.Table(
        columns=["Image"] + _to_str(class_labels.values()),
    )
...
class TrafficLightIOU(IOU):
    nm = "traffic light"


class TrafficSignIOU(IOU):
    nm = "traffic sign"
...
class VehicleIOU(IOU):
    nm = "vehicle"


class BicycleIOU(IOU):
    nm = "bicycle"
...
class IOUMacro(DiceMulti):
    ...
    def value(self):
        ...
        if c not in self.count:
            ...
            return self.macro[c] / self.count[c] if self.count[c] > 0 else np.nan

    def reset(self):
        self.macro, self.count = {}, {}
...
class MIouMacro(IOUMacro):
    ...
    def value(self):
        ...
        for c in self.count:
            binary_iou_scores = np.append(
                binary_iou_scores, self.macro[c] / self.count[c] if self.count[c] > 0 else np.nan
            )
        return np.nanmean(binary_iou_scores)


class BackgroundIouMacro(IOUMacro):
    nm = "background"


class RoadIouMacro(IOUMacro):
    nm = "road"


class TrafficLightIouMacro(IOUMacro):
    nm = "traffic light"


class TrafficSignIouMacro(IOUMacro):
    nm = "traffic sign"


class PersonIouMacro(IOUMacro):
    nm = "person"


class VehicleIouMacro(IOUMacro):
    nm = "vehicle"


class BicycleIouMacro(IOUMacro):
    nm = "bicycle"
...
def display_diagnostics(learner, dls=None, return_vals=False):
    ...
    classes = list(cfg.dataset.classes.values())
    y_true = targs.flatten().numpy()
    y_pred = preds.flatten().numpy()

    tdf, pdf = (
        pd.DataFrame(r).value_counts().to_frame(c)
        for r, c in zip((y_true, y_pred), ["y_true", "y_pred"])
    )
    countdf = (
        tdf.join(pdf, how="outer")
        .reset_index(drop=True)
        .fillna(0)
        .astype(int)
        .rename(index=cfg.dataset.classes)
    )
    ...
    disp = ConfusionMatrixDisplay.from_predictions(
        y_true=y_true, y_pred=y_pred, display_labels=classes, normalize="pred"
    )
    fig = disp.ax_.get_figure()
    fig.set_figwidth(10)
    fig.set_figheight(10)
    disp.ax_.set_title(
        "Confusion Matrix (by Pixels)", fontdict={"fontsize": 32, "fontweight": "medium"}
    )
    fig.show()
